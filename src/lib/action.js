...
export const saveSubscriptionDetails = async (
  data,
  userId,
  plan,
  planId,
  ...
  try {
    ...
    if (payment) {
      payment.Subscription = {
        ...
        nextBilledAt: currentDate.setMonth(
          currentDate.getMonth() + (billingCycle === "monthly" ? 1 : 12)
        ),
        endDate: null,
      };
    } else {
...
export const calculatePrice = (portfolioData) => {
  ...
  const assetRanges = {
    "1-15": [
      {
        range: ["10.000 - 25.000 €", "25.000 - 50.000 €", "50.000 - 100.000 €"],
        price: 995,
      },
      { range: ["100.000 - 200.000 €"], price: 1295 },
      {
        range: ["200.000 - 400.000 €", "400.000 - 750.000 €", "750.000+"],
        price: 1495,
      },
    ],
    "16-25": [
...
export const importPortfolios = async () => {
  try {
    ...
    const portfolios = jsonData.find(
      (item) => item.type === "table" && item.name === "Portfolios"
    ).data;
...
const processBatch = async (batch) => {
  const operations = batch.map(async (portfolio) => {
    const existingUser = await PastUsers.findOne({
      Name: portfolio.Name,
    }).exec();
    if (!existingUser) {
...
export const importPortfolioAssets = async () => {
  try {
    ...
    const portfolios = jsonData.find(
      (item) => item.type === "table" && item.name === "Portfolio_Assets"
    ).data;
...
const processBatch2 = async (batch) => {
  const operations = batch.map(async (portfolio) => {
    if (portfolio.PortfolioID && portfolio.ID && portfolio.AssetID) {
      const existingPortfolio = await PastPortfolio.findOne({
        ID: portfolio.ID,
      }).exec();
    ...
    while (index < jsonData[2].data.length) {
      const batchData = jsonData[2].data
        ...
        .map((item) => ({
          ...
          PricePerCoin: isNaN(parseFloat(item.PricePerCoin))
            ? 0
            : parseFloat(item.PricePerCoin),
