...
async function updateCoinDetails(coinGeckoID) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            console.error(`Error retrieving price data for ${coinGeckoID}. HTTP Status Code: ${response.status}`);
        }
    } catch (error) {
...
export async function POST(req) {
    ...
    try {
        ...
        if (responsePing.ok) {
            ...
            const promises = coinGeckoIDs.map(coinGeckoID => updateCoinDetails(coinGeckoID).catch(error => console.error(`Failed to update details for ${coinGeckoID}: ${error.message}`)));
            await Promise.allSettled(promises);
            console.log("All coin details updated successfully.");
            return new NextResponse(JSON.stringify({ message: "All coin details updated successfully" }), { status: 200 });
        } else {
            console.error("Failed to ping CoinGecko API");
            return new NextResponse(JSON.stringify({ message: "Failed to ping CoinGecko API" }), { status: 500 });
        }
    } catch (e) {
        console.error("Error in GET function:", e);
        return new NextResponse(JSON.stringify({ message: `Error occurred updating coin data ${e.message}` }), { status: 500 });
    }
}
