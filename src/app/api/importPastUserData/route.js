  ...
  try {
    ...
    if (user.pastUserCheck) {
      return NextResponse.json(
        {
          message:
            "You can't import previous user data, contact admin for details",
        },
        { status: 403 }
      );
    }

    const pastUser = await PastUsers.findOne({
      Name: { $regex: new RegExp("^" + Name + "$", "i") },
    }).exec();
    if (!pastUser) {
      return NextResponse.json(
        { message: "Past user not found" },
        { status: 404 }
      );
    }
    ...
    if (pastUser.ImportedSuccessfully) {
      return NextResponse.json(
        { message: "This User is already Imported" },
        { status: 401 }
      );
    }
    ...
    const portfolios = await PastPortfolio.find({
      PortfolioID: pastUser.Name,
    }).exec();
    if (!portfolios.length) {
      return NextResponse.json(
        { message: "No portfolios found for this past user" },
        { status: 404 }
      );
    }
    ...
    const buyAndSellData = await PastBuyAndSell.find({
      PortfolioAssetID: { $in: portfolios.map((p) => p.ID) },
    }).exec();
    ...
    const userPortfolios = portfolios
      .map((portfolio) => {
        ...
        const adjustedDCA =
          portfolio.DCA !== "" ? Number(portfolio.DCA) + 1 : 0;
        const adjustedGewichtung =
          portfolio.Gewichtung !== "" ? Number(portfolio.Gewichtung) + 1 : 0;
        const adjustedRelevanz =
          portfolio.Relevanz !== "" ? Number(portfolio.Relevanz) + 1 : 0;
        const Price = assetMap[portfolio.AssetID]
          ? assetMap[portfolio.AssetID].Price
          : 0;
        const totalHoldingsValue = (totalCoins * parseFloat(Price)).toFixed(2);

        const assets = [
          {
            CoinGeckoID: assetMap[portfolio.AssetID]
              ? assetMap[portfolio.AssetID].CoinGeckoID
              : null,
