...
export async function POST(req) {
  ...
  try {
    ...
    //   `https://pro-api.coingecko.com/api/v3/coins/list?x_cg_pro_api_key=${process.env.NEXT_PUBLIC_COINGECKO_API_KEY}`
    // );
    // const coins = await reponse.json();
    // console.log(coins);
    // const coin = coins.find(
    //   (c) => c.name.toLowerCase() === data.name.toLowerCase()
    // );
    // // console.log(
    // //   coins.find((c) => c?.name.toLowerCase().includes("vectorspace ai"))
    // // );
    // console.log(coin);

    // const respCoin = await fetch(
    //   `https://pro-api.coingecko.com/api/v3/coins/${coin?.id}?x_cg_pro_api_key=${process.env.NEXT_PUBLIC_COINGECKO_API_KEY}`
    // );
    // const coinData = await respCoin.json();
    // console.log(coinData?.name, coinData?.Ticker, coinData?.Bottom);
